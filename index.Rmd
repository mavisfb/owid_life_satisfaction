...
## Import data
library(readr)
happiness_cantril_ladder <- read_csv("data/happiness-cantril-ladder.csv")
View(happiness_cantril_ladder)
```
...
happiness_cantril_ladder%>% 
  ...
  pivot_wider(names_from = Year, values_from = `Cantril ladder score`, id_cols = Entity) %>% 
  select(Entity, `2011`, `2022`) %>% 
...
cantril_chart <- 
 happiness_cantril_ladder  %>% 
  filter(Entity %in% c("Norway", "Australia", "Germany", "Bolivia", "Colombia", "Bhutan")) %>%
  ggplot(aes(x = Year, y = `Cantril ladder score`, color = Entity)) +
  geom_line() +
...
shared_cantril <- highlight_key(happiness_cantril_ladder)
